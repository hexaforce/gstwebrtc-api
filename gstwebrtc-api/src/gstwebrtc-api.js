...
function registerConnectionListener(listener) {
  if (!listener || typeof listener !== 'object' || typeof listener.connected !== 'function' || typeof listener.disconnected !== 'function') {
    return false
  }
...
function registerProducersListener(listener) {
  if (!listener || typeof listener !== 'object' || typeof listener.producerAdded !== 'function' || typeof listener.producerRemoved !== 'function') {
    return false
  }
...
function connectChannel() {
  ...
  apiState.channel = new ComChannel(apiState.config.signalingServerUrl, apiState.config.meta, apiState.config.webrtcConfig)
